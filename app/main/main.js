...
/**
 * Icon path for BrowserWindow + Dock.
 * KodanFlower.icns is often ic12; Electron frequently fails to load it for the Dock (rejected Promise).
 * Prefer KodanFlower-dock.png (1024px, exported from the .icns via sips) when present.
 */
...
function createGreyPNG(width, height, filePath) {
  return new Promise((resolve, reject) => {
    ...
    if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0 || w > 16384 || h > 16384) {
      reject(new Error(`createGreyPNG: invalid dimensions (${width}, ${height})`));
      return;
    }
...
function calculateCRC(buf) {
  ...
  // >>> 0: PNG CRC must be written as uint32; plain ^ can yield a signed negative in JS.
...
ipcMain.handle('read-project-kodan', async (event, projectFilePath) => {
