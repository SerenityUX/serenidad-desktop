...
const FROM_EMAIL = "CoCreate Cafe <noreply@cc.serenidad.app>";
...
async function sendOtpEmail(to, code) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: "Your CoCreate Cafe verification code",
...
async function sendProjectShareEmail({ to, projectName, sharerName, sharerEmail }) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: `${sharerName || sharerEmail || "Someone"} shared a project with you on CoCreate Cafe`,
    html: `<p>${who} shared the project <strong>${projectSafe}</strong> with you on CoCreate Cafe.</p><p>Open the app — it will appear in your projects list and you can make changes alongside them.</p>`,
  });
  if (error) {
    throw new Error(error.message || "Resend send failed");
  }
...
async function sendProjectInviteSignupEmail({ to, sharerName, sharerEmail }) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: `${whoPlain} sent you an invite to CoCreate Cafe`,
    html: `<p>${who} sent you an invite to CoCreate Cafe. Go create an account and you can begin telling the story with ${who}.</p><p><a href="https://serenidad.app">https://serenidad.app</a></p>`,
  });
  if (error) {
    throw new Error(error.message || "Resend send failed");
  }
