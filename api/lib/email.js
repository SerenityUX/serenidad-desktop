...
const FROM_EMAIL = "CoCreate <noreply@cc.serenidad.app>";
...
async function sendOtpEmail(to, code) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: "Your CoCreate verification code",
...
async function sendProjectShareEmail({ to, projectName, sharerName, sharerEmail }) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: `${sharerName || sharerEmail || "Someone"} shared a project with you on CoCreate`,
    html: `<p>${who} shared the project <strong>${projectSafe}</strong> with you on CoCreate.</p><p>Open the app — it will appear in your projects list and you can make changes alongside them.</p>`,
  });
  if (error) {
    throw new Error(error.message || "Resend send failed");
  }
...
async function sendProjectInviteSignupEmail({ to, sharerName, sharerEmail }) {
  ...
  const { error } = await resend.emails.send({
    from,
    to: [to],
    subject: `${whoPlain} sent you an invite to CoCreate`,
    html: `<p>${who} sent you an invite to CoCreate. Go create an account and you can begin telling the story with ${who}.</p><p><a href="https://serenidad.app">https://serenidad.app</a></p>`,
  });
  if (error) {
    throw new Error(error.message || "Resend send failed");
  }
